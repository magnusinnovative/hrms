...
def get_user_branch():
    """Get branch of the currently logged-in user's employee record"""
...
def get_columns_for_days(filters: Filters) -> list[dict]:
    days = []
    for d in get_dates_in_period(filters):
        d = getdate(d)
...
def get_dates_in_period(filters: Filters) -> list[str]:
    if filters.filter_based_on == "Month":
        ...
        return [
            f"{cstr(filters.year)}-{cstr(filters.month)}-{cstr(day)}"
            for day in range(1, total_days + 1)
        ]
    if filters.filter_based_on == "Date Range":
        return get_date_range(filters.start_date, filters.end_date)
    return []
...
def get_holiday_map(filters: Filters) -> dict:
...
def get_rows(employee_details, filters, holiday_map, attendance_map) -> list[dict]:
    records = []
    default_holiday_list = frappe.get_cached_value("Company", filters.company, "default_holiday_list")
...
def get_attendance_status_for_summarized_view(
    employee, filters, holidays, joined_in_current_period, joined_date
) -> dict:
...
def get_attendance_summary_and_days(employee, filters) -> tuple:
    Attendance = frappe.qb.DocType("Attendance")

    present_case = frappe.qb.terms.Case().when(
        ((Attendance.status == "Present") | (Attendance.status == "Work From Home")), 1
    ).else_(0)
    ...
    summary = (
        frappe.qb.from_(Attendance)
        .select(
            Sum(present_case).as_("total_present"),
            Sum(absent_case).as_("total_absent"),
            Sum(leave_case).as_("total_leaves"),
            Sum(half_day_case).as_("total_half_days"),
        )
        .where(
            (Attendance.docstatus == 1)
...
def get_attendance_status_for_detailed_view(employee, filters, employee_attendance, holidays) -> list[dict]:
    total_days = get_dates_in_period(filters)
    attendance_values = []
...
def get_holiday_status(holiday_date, holidays) -> str:
    if holidays:
        for holiday in holidays:
            if holiday_date == holiday.get("holiday_date"):
                return "Weekly Off" if holiday.get("weekly_off") else "Holiday"
    return None


def get_leave_summary(employee, filters) -> dict:
    Attendance = frappe.qb.DocType("Attendance")
    ...
    leave_details = (
        frappe.qb.from_(Attendance)
        .select(Attendance.leave_type, Sum(day_case).as_("leave_days"))
        .where(
            (Attendance.employee == employee)
            & (Attendance.docstatus == 1)
            & (Attendance.company.isin(filters.companies))
    ...
    return {frappe.scrub(d.leave_type): d.leave_days for d in leave_details}


def get_entry_exits_summary(employee, filters) -> dict:
    Attendance = frappe.qb.DocType("Attendance")
    attendance_date_condition = get_date_condition(Attendance.attendance_date, filters)
    ...
    entry_exits = (
        frappe.qb.from_(Attendance)
        .select(
            Count(late_entry_case).as_("total_late_entries"),
            Count(early_exit_case).as_("total_early_exits"),
        )
        .where(
            (Attendance.docstatus == 1)
            & (Attendance.employee == employee)
...
@frappe.whitelist()
def get_logged_user_branch() -> str:
    """Returns branch of the currently logged-in user's employee record"""
    branch = frappe.db.get_value(
        "Employee",
        {"user_id": frappe.session.user},
        "branch"
...
def get_chart_data(attendance_map, filters) -> dict:
